<template>
  <div class="page" :class="pageClasses">
    <div class="page-content">
      <h2 class="page-header">{{ $props.header }}</h2>
      <div
        class="page-text"
        v-html="$props.text"
        :style="$props.fontSize"
      ></div>
      <div class="page-footer">{{ $props.pageNum }}</div>
    </div>
  </div>
</template>
...
.page {
  ...
  .page-content {
    ...
    //For delimiter ***
    .page-text ::v-deep(.delimiter) {
      line-height: 1.6em;
      width: 100%;
      text-align: center;
    }

    .page-text ::v-deep(.delimiter):before {
    ...
    .page-text ::v-deep(.img-credits a) {
      color: black;
      background: none;
    }

    .page-text ::v-deep(.img-caption) {
      text-align: center;
      font-size: 0.8em;
      margin-top: 5px;
    }
